...
### Decision: Decoupled Message History Management

* **Reason:** To allow for different persistence strategies (e.g., In-Memory for ephemeral sessions, Database for persistent chats) and to centralize conversation state logic.
* **Implications:**
    * All conversation state must be managed via the `IMessageHistory` interface.
    * Services should accept arrays of `GenericMessage` rather than maintaining their own internal state.
    * This separation ensures that the `ILlmService` implementations remain stateless regarding the conversation history.
