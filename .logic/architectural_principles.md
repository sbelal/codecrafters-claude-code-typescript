...
### Principle: Provider-Agnostic Abstractions

* **Rationale:** To ensure the system remains flexible and can easily integrate with different LLM providers (OpenAI, Anthropic, etc.) without rewriting core business logic or history management.
* **Implications:**
    * Core interfaces (like `ILlmService`) and data structures (like `GenericMessage`) must remain independent of any specific provider SDK.
    * Use a mapping layer within specific service implementations to translate between these generic types and provider-specific types.
